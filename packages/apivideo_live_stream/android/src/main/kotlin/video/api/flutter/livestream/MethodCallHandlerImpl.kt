    ...
    override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
        when (call.method) {
            ...
            "startStreaming" -> {
                ...
                val fullUrl = call.argument<String>("fullUrl")
                val targetUrl = when {
                    !fullUrl.isNullOrBlank() -> fullUrl
                    streamKey != null && streamKey.isNotEmpty() && !url.isNullOrBlank() ->
                        url.addTrailingSlashIfNeeded() + streamKey
                    else -> null
                }
                when {
                    targetUrl.isNullOrBlank() -> result.error(
                        "missing_rtmp_url",
                        "RTMP URL is missing",
                        null,
                    )
                        ...
                        try {
                            flutterView!!.startStream(
                                targetUrl,
                                onSuccess = { result.success(null) },
                                onError = {
                                    result.error("failed_to_start_stream", it.message, null)
                                },
                            )
                        } catch (e: Exception) {
            ...
            "stopStreaming" -> {
                ...
                if (view == null) {
                    ...
                    view.stopStream(onComplete = { result.success(null) })
                }
            }

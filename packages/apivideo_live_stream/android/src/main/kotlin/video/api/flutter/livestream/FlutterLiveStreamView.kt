...
class FlutterLiveStreamView(
    ...
    private var _awaitingPublishResult = false
    ...
    fun startStream(
        ...
        streamScope.launch {
            try {
                if (!_isPreviewing) {
                    throw IllegalStateException("Camera preview is not running")
                }
    ...
    fun stopStream(onComplete: (() -> Unit)? = null) {
        streamScope.launch {
            val wasConnected = streamer.isConnected
            try {
                streamer.stopStream()
                streamer.disconnect()
            } catch (_: Exception) {
